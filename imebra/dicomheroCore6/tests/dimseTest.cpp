...
TEST(dimseTest, dimseTimeoutTest)
{
    ...
    const std::int64_t seconds = std::chrono::duration_cast<std::chrono::seconds>(endTime - startTime).count();
    EXPECT_TRUE(seconds > 9 && seconds < 12);

#if defined(DICOMHERO_EXCEPTIONS_TRACKING) && defined(DICOMHERO_LOGGING_LOG4CXX)
...
TEST(dimseTest, storeSCUInteroperabilityTest)
{
    ...
    if (pID == 0)                // child
    {
        execlp("storescp", "storescp", "-v", "+v", "-d", "-od", dirName.c_str(), "30003", nullptr);
    }
...
TEST(dimseTest, storeSCPInteroperabilityTest)
{
    ...
    TCPListener tcpListener(TCPPassiveAddress("127.0.0.1", "30004"));
    ...
    for(int i=0; i != 20; ++i )
    {
        ...
        ASSERT_THROW(dimse.getCommand(), StreamClosedError);
    }
    thread.join();
...
TEST(dimseTest, moveSCPInteroperabilityTest)
{
    ...
    {
        TCPListener tcpListener(TCPPassiveAddress("127.0.0.1", "30005"));
        TCPStream tcpStream(tcpListener.waitForConnection());
...
TEST(dimseTest, findSCPInteroperabilityTest)
{
...

    {
        ...
        DataSet study0(dimse.getTransferSyntax(find.getAbstractSyntax()));
        study0.setString(TagId(tagId_t::PatientID_0010_0020), "100");
        study0.setString(TagId(tagId_t::StudyID_0020_0010), "110");
        dimse.sendCommandOrResponse(CFindResponse(find, study0));

        DataSet study1(dimse.getTransferSyntax(find.getAbstractSyntax()));
        study1.setString(TagId(tagId_t::PatientID_0010_0020), "100");
        study1.setString(TagId(tagId_t::StudyID_0020_0010), "120");
