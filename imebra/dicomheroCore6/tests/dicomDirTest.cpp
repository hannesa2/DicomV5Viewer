...
TEST(dicomDirTest, createDicomDir)
{
    ...
    DicomDirEntry rootRecord = newDicomDir.getNewEntry("PATIENT");

    DataSet rootRecordDataSet(rootRecord.getEntryDataSet());
    rootRecordDataSet.setUnicodeString(TagId(tagId_t::PatientName_0010_0010), L"Surname");
    newDicomDir.setFirstRootEntry(rootRecord);

    DicomDirEntry nextRecord = newDicomDir.getNewEntry("PATIENT");
    DataSet nextRecordDataSet = nextRecord.getEntryDataSet();
    nextRecordDataSet.setUnicodeString(TagId(tagId_t::PatientName_0010_0010), L"Surname 1");
    rootRecord.setNextEntry(nextRecord);

    DicomDirEntry imageRecord = newDicomDir.getNewEntry("IMAGE");
    DataSet imageRecordDataSet = imageRecord.getEntryDataSet();
