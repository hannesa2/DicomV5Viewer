...
{
    TCPActiveAddress connectToAddress("127.0.0.1", port);
...
TEST(tcpTest, sendReceive)
{
    ...
    TCPPassiveAddress listeningAddress("127.0.0.1", listeningPort);
...
{
    TCPPassiveAddress listeningAddress("127.0.0.1", port);
...
TEST(tcpTest, prematureClose)
{
    ...
    const size_t numTrials = bFastMode ? 1u : 10u;
    ...
    for(size_t trial(0); trial < numTrials; ++trial)
    {
        ...
        TCPActiveAddress connectToAddress("127.0.0.1", listeningPort);
        ...
        {
            std::cout << "PrematureClose exited with unexpected exception " << e.what() << std::endl;
            EXPECT_TRUE(false);
        }
...
{
    TCPActiveAddress connectToAddress("127.0.0.1", "20000");
