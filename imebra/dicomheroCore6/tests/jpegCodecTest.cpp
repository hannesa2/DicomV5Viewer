...
TEST(jpegCodecTest, testBaseline)
{
    for(int precision=0; precision != 2; ++precision)
    {
        ...
        Image ybrImage = transformToYBR.allocateOutputImage(baselineImage, width, height);
        ...
        Image rgbImage = transformToRGB.allocateOutputImage(checkImage, checkWidth, checkHeight);
