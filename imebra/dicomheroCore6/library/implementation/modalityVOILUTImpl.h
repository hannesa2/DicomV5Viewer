                    ...
                    std::uint32_t outputTopLeftX, std::uint32_t outputTopLeftY) const
	{
        ...
        if(m_voiLut != 0 && m_voiLut->getSize() != 0)
		{
			for(; inputHeight != 0; --inputHeight)
			{
				...
				{
                    *(pOutputMemory++) = static_cast<outputType>( m_voiLut->getMappedValue(static_cast<std::int32_t>(*(pInputMemory++))));
				}
		...
		for(; inputHeight != 0; --inputHeight)
		{
            for(std::uint32_t scanPixels(inputWidth); scanPixels != 0; --scanPixels)
			{
                *(pOutputMemory++) = static_cast<outputType>(static_cast<double>(*(pInputMemory++)) * m_rescaleSlope + m_rescaleIntercept);
			}
			pInputMemory += (inputHandlerWidth - inputWidth);
			pOutputMemory += (outputHandlerWidth - inputWidth);
		}
