...
public:
    ...
    ///
    ///////////////////////////////////////////////////////////
    VOILUT(): m_pLUT(nullptr), m_windowCenter(0), m_windowWidth(0), m_function(dicomVOIFunction_t::linear)
    {}
...

    {
        ...
        //
        ///////////////////////////////////////////////////////////
        const std::int64_t outputHandlerNumValues = (std::int64_t)1 << (outputHighBit + 1);
        const std::int64_t outputMin(outputHandlerMinValue);
        const std::int64_t outputMax(outputHandlerMinValue + outputHandlerNumValues - 1);
        const double outputMaxMinusOutputMin = static_cast<double>(outputMax - outputMin);
        const double outputMinDouble = static_cast<double>(outputMin);
        ...
        case dicomVOIFunction_t::linearExact:
            {
                for(; inputHeight != 0; --inputHeight)
                {
                    for(std::uint32_t scanPixels(inputWidth); scanPixels != 0; --scanPixels)
                    {
                        const double inputValue = static_cast<double>(*(pInputMemory++));
                        const std::int64_t outputValue = static_cast<std::int64_t>(((inputValue - m_windowCenter) / m_windowWidth) * outputMaxMinusOutputMin + outputMinDouble);
        ...
        case dicomVOIFunction_t::sigmoid:
            {
                for(; inputHeight != 0; --inputHeight)
                {
                    ...
                    {
                        const double inputValue = static_cast<double>(*(pInputMemory++));
                        const std::int64_t outputValue = static_cast<std::int64_t>(outputMaxMinusOutputMin/(1.0 + std::exp(-4.0 * (inputValue - m_windowCenter)/ m_windowWidth)));
                        if(outputValue < outputMin)
                        {
                            *pOutputMemory++ = (outputType)outputMin;
                        }
        ...
        case dicomVOIFunction_t::linear:
            {
                ...
                else
                {
                    for(; inputHeight != 0; --inputHeight)
                    {
                        ...
                        {
                            const double inputValue = static_cast<double>(*(pInputMemory++));
                            const std::int64_t outputValue = (std::int64_t)(((inputValue - m_windowCenter + 0.5) / (m_windowWidth - 1.0) + 0.5) * outputMaxMinusOutputMin + outputMinDouble);
                            if(outputValue < outputMin)
                            {
                                *pOutputMemory++ = (outputType)outputMin;
                            }
...
protected:
                    ...
                    std::shared_ptr<VOIDescription>& voiDescription)
    {
        ...
        const double center = static_cast<double>(static_cast<std::int64_t>(maxValue) + static_cast<std::int64_t>(minValue)) / 2.0;
        const double width = 2.0 * (center - static_cast<double>(minValue));
    ...
    const std::shared_ptr<const lut> m_pLUT;
    const double m_windowCenter;
    const double m_windowWidth;
