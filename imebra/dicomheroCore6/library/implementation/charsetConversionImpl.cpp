...
std::string dicomConversion::convertFromUnicode(const std::wstring& unicodeString, const charsetsList_t& charsets)
{
    ...
    if(charsets.empty())
    {
        const charsetInformation info = defaultCharsetConversion::getDictionary().getCharsetInformation(charsetDictionary::getDefaultCharset(), 0);
    ...
    for(size_t scanString = 0; scanString != unicodeString.size(); ++scanString)
    {
        ...
        ///////////////////////////////////////////////////////////
        if(sizeof(wchar_t) == 2u) // warning C4127: conditional expression is constant on Windows. Need C++17 constexpr to remove
        {
