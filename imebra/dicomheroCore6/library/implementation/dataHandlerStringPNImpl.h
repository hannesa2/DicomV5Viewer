...
private:
    ...
    template<class stringType_t, class patientNameType_t, typename separatorType_t> std::shared_ptr<patientNameType_t> returnPatientName(const stringType_t& string, separatorType_t separator) const
    {
        ...
        for(size_t startPosition(0); startPosition != string.size(); /* in the loop */)
        {
            size_t endPosition = string.find(separator, startPosition);
            if(endPosition == std::string::npos)
            {
                groups.push_back(string.substr(startPosition));
                break;
            }
            groups.push_back(string.substr(startPosition, endPosition - startPosition));
